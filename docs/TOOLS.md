...
(Unity, Godot, Unreal). Skips files whose contents are unchanged; writes
...
| `project_dir` | `Path` | required | Engine project root. Unity: folder with `Assets/`. Unreal: folder with `Content/`. Godot: folder with `project.godot`. |
| `engine` | `"unity"` / `"godot"` / `"unreal"` | required | Target engine. |
...
**Why no sidecars?** Unity's `.meta`, Godot's `.import`, and Unreal's
