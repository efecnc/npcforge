...
//
// Attach to a Unity Button. Pick an NPC id, pick a dialogue mode in the
// Inspector, and the button plays the right generated Yarn node. The mode
// maps one-to-one to npcforge's generator outputs:
//
//   TimeOfDayGreeting → <npc>_Greet_time_of_day   (from `gen greetings`)
//   RepeatGreeting    → <npc>_RepeatGreet         (from `gen repeat-greeting`)
//   WalkUp            → <npc>                     (from `build --mode walk_up`)
